...
RUN set -e; \
    if [ -f "package.json" ]; then \
      ...
      npm install --legacy-peer-deps || npm install || { \
        echo "warn: install failed (bad versions?), normalising all deps to latest and retrying..."; \
        node -e "var fs=require('fs'),p=JSON.parse(fs.readFileSync('package.json','utf8'));['dependencies','devDependencies','peerDependencies'].forEach(function(k){if(p[k])Object.keys(p[k]).forEach(function(n){p[k][n]='latest'})});fs.writeFileSync('package.json',JSON.stringify(p,null,2))"; \
        npm install --legacy-peer-deps || npm install; \
      }; \
